...
namespace
{
	size_t powerTraceSlots()
	{
		return 3 * NoximGlobalParams::mesh_dim_x *
			   NoximGlobalParams::mesh_dim_y *
			   NoximGlobalParams::mesh_dim_z;
	}

	void ensurePowerTraceSlots(vector<double>& trace)
	{
		const size_t expected = powerTraceSlots();
		if (trace.size() < expected)
			trace.resize(expected, 0.0);
	}
}
...
void NoximNoC::buildMesh()
{
    ...
    nnmodel.load();	//tytyty // 读取model.txt, weight.txt等
	//解析所有层的结构参数
	//加载权重到 `all_conv_weight` 和 `all_conv_bias`
	//加载映射表到 `mapping_table`
	//构建 `Group_table` 和 `mapping_table`
	...
	for ( i = 0; i < NoximGlobalParams::mesh_dim_x; i++) {
		for ( j = 0; j < NoximGlobalParams::mesh_dim_y; j++) {
			for( k = 0; k < NoximGlobalParams::mesh_dim_z; k++) {
	    		...
	    		t[i][j][k]->pe->NN_Model = &nnmodel;	//tytyty// 关键: 模型指针传递
...
bool NoximNoC::isInferenceDone() const
{
	if (NoximGlobalParams::time_div_mul < 0 ||
	...
	if (wr >= (int)nnmodel.Group_table.size())
	{
		return false;
	}

	if (nnmodel.all_data_in.empty())
	{
		return false;
	}

	const int pic_size = nnmodel.all_data_in[0].size();
	if (pic_size <= 0)
	{
		return false;
	}

	const int group_count = nnmodel.Group_table[wr].size();
	if (group_count <= 0)
	{
		return false;
	}

	const long long expected_count = (long long)pic_size * (long long)group_count;
	const bool is_last_tdm_phase = (wr == NoximGlobalParams::tdm - 1);

	return is_last_tdm_phase &&
		NoximGlobalParams::tdm_flag &&
		((long long)NoximGlobalParams::count_PE >= expected_count);
}

bool NoximNoC::isPipelineIdle() const
{
	for (int z = 0; z < NoximGlobalParams::mesh_dim_z; z++)
	{
		for (int y = 0; y < NoximGlobalParams::mesh_dim_y; y++)
...
{ 
	...
	{
	  	if(NoximGlobalParams::thermal_update == 1 &&
		   ( getCurrentCycleNum() % (int) (TEMP_REPORT_PERIOD) ) == 0 )
	  	{
	...
	if (!reset.read() && NoximGlobalParams::stop_on_infer_done == 1)
	{
		static bool inference_stop_issued = false;
		if (!inference_stop_issued && isInferenceDone() && isPipelineIdle())
		{
			inference_stop_issued = true;
			NoximGlobalParams::stop_by_infer_done = 1;
			cout << "[AutoStop] inference completed at cycle "
				 << getCurrentCycleNum()
				 << ", stopping simulation early." << endl;
			sc_stop();
		}
	}
}
...
{
	ensurePowerTraceSlots(instPowerTrace);
	int idx = 0;	
	int m, n, o;
...
{	
	ensurePowerTraceSlots(overallPowerTrace);
    int idx = 0;	
	int m, n, o;
	/*================================Begin of collecting POWER TRACE ======================================*/
	for(o=0; o < NoximGlobalParams::mesh_dim_z; o++){
...
{
	ensurePowerTraceSlots(TemperatureTrace);
	int m, n, o;
    int idx = 0;
	for(o=0; o < NoximGlobalParams::mesh_dim_z; o++){
		for(n=0; n < NoximGlobalParams::mesh_dim_y; n++) {
