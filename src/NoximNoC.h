...
SC_MODULE(NoximNoC)
{
	...
	// int	cnt_neighbor_total[4];//一 层所有 router 转发到邻居方向的 flit 总数累加
	// int cnt_received_total[4];//一层所有 PE 接收的 flit 总数累加。
	// int	cnt_local_total[4];//一层所有 PE 在一个统计周期内本地注入的 flit 总数累加
	// int	tot_cnt_local[4][4][4];//单个坐标 (x,y,z) 的本地注入计数历史累加
	// int tot_cnt_neighbor[4][4][4];//单个坐标 (x,y,z) 的邻居转发计数历史累加。
	// int tot_cnt_received[4][4][4];//单个坐标 (x,y,z) 的接收计数历史累加。
	//modify by chunyu
	int	cnt_neighbor_total[16];
	int cnt_received_total[16];
	int	cnt_local_total[16];
	int	tot_cnt_local[16][16][16];
	int tot_cnt_neighbor[16][16][16];
	int tot_cnt_received[16][16][16];
	//end modify
